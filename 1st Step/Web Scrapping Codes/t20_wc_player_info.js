...
allRows.each((index, element) => {
  ...
  const rowURL =
    "https://www.espncricinfo.com" + $(tds[6]).find("a").attr("href");
  links.push(rowURL);
});
return {
...
var match = $("div")
  .filter(function () {
    return (
      $(this).find("span > span > span").text() === String("Match Details")
    );
  })
  .siblings();
team1 = $(match.eq(0))
...
var firstInningRows = $(tables.eq(0))
  .find("tbody > tr")
  .filter(function (index, element) {
    return $(this).find("td").length >= 8;
  });

var secondInningsRows = $(tables.eq(1))
  .find("tbody > tr")
  .filter(function (index, element) {
    return $(this).find("td").length >= 8;
  });
...
var firstInningRows = $(tables.eq(1))
  .find("tbody > tr")
  .filter(function (index, element) {
    return $(this).find("td").length >= 11;
  });

var secondInningsRows = $(tables.eq(3))
  .find("tbody > tr")
  .filter(function (index, element) {
    return $(this).find("td").length >= 11;
  });
